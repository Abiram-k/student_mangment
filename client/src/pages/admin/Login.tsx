...
export default function AdminPortalLogin() {
  ...
  const { useAdminMutation } = useAdmin();
  const { mutate: adminLogin } = useAdminMutation();
  ...
  const validateForm = (): Boolean => {
    ...
    if (!formData.password.trim() || formData.password.length < 6) {
      newErrors.password = "Check your password .";
    }
    setErrors(newErrors);
      ...
      { email: formData.email, password: formData.password },
      {
        onSuccess: (data: any) => {
          ...
          if (data[0]?.id) {
            ...
            navigate("/student/admin/dashboard",{replace:true});
